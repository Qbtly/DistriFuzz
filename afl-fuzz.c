...
#define MUTATE_FUNC_COUNT (sizeof(mutate_funcs) / sizeof(MutateFunc))
 ...
 #define NOVEL_MAX_ESTIMATE 65536.0
 #define MMD_MAX_ESTIMATE   8192.0
 // #define NOVEL_MAX_ESTIMATE 6553.6
 // #define MMD_MAX_ESTIMATE   1638.4
 ...
 static int init_py() {
   ...
   if (module_name) {
     ...
     if (py_module != NULL) {
       ...
       py_functions[PY_FUNC_PARSE] = PyObject_GetAttrString(py_module, "parse");
 ...
 static void fuzz_py(char** ret, size_t* retlen) {
 ...
 static int parse_py(char* buf, size_t buflen, char* add_buf, size_t add_buflen) {
   PyObject *py_args, *py_value;
   ...
   if (py_module != NULL) {
     ...
     if (py_value != NULL) {
       int num_of_new_samples = PyLong_AsLong(py_value);
       //printf("size in c :%d", num_of_new_samples);
       Py_DECREF(py_value);
       return num_of_new_samples;
     } else {
       PyErr_Print();
       fprintf(stderr,"Call failed\n");
       return 0;
...
if (saved) interesting_tc_count++;  // 累计全局计数
 ...
 void print_testcase_preview(int index, int parent_id, TestCase* tc) {
   if (!tc || !tc->data) {
     printf("  [\033[0;35mTestcase %d\033[0m] From Parent %d | Invalid testcase\n", index, parent_id);
     return;
   }
 
   int preview_len = (tc->size < 8) ? tc->size : 8;
   printf("  [\033[0;35mTestcase %d\033[0m] From Parent %d | Size: %zu | First bytes: ", index, parent_id, tc->size);
   for (int b = 0; b < preview_len; b++) {
     printf("%02X ", (unsigned char)tc->data[b]);
   }
   if (tc->size > preview_len) printf("...");
   printf("\n");
 }
 ...
 void init_fitness_log_file() {
   ...
   if (log_env) {
     ...
     FITNESS_LOG_FILE = strdup("fitness_log(mmd).csv");  // 默认值也要 strdup
     printf("[+] Using default fitness log file: %s\n", FITNESS_LOG_FILE);
   }
 
   // 如果文件存在，先删除
   if (access(FITNESS_LOG_FILE, F_OK) == 0) {
     if (remove(FITNESS_LOG_FILE) == 0) {
       printf("[+] Existing log file deleted.\n");
     } else {
       perror("[-] Failed to delete existing log file");
     }
   }
 
   // 创建空文件
   FILE* fp = fopen(FITNESS_LOG_FILE, "w");
   if (!fp) {
 ...
 // 简化版 MMD 距离计算：E[XX] + E[YY] - 2E[XY]
 double compute_avg_mmd_distance(Individual* indiv, Individual* seed) {
   if (!indiv || !seed || !seed->coverage_r) return 0.0;
   ...
   for (int i = 0; i < m; i++) {
     for (int j = 0; j < m; j++) {
       for (int d = 0; d < dim; d++) {
         uint8_t a = indiv->coverage_r[i][d] > 0;
         uint8_t b = indiv->coverage_r[j][d] > 0;
         sum_xx += (a * b);
       }
     }
   }
   ...
   for (int i = 0; i < n; i++) {
     for (int j = 0; j < n; j++) {
       for (int d = 0; d < dim; d++) {
         uint8_t a = seed->coverage_r[i][d] > 0;
         uint8_t b = seed->coverage_r[j][d] > 0;
         sum_yy += (a * b);
       }
     }
   }
 ...
 double compute_combined_fitness(Individual* indiv, int index, Individual* seed) {
   ...
   double total_novel = 0.0, mmd = 0.0;
   double norm_novel = 0.0, norm_mmd = 0.0;
 
   if (g_alpha > 1e-6) {
     total_novel = compute_total_novel_bits(indiv, index, seed);
     if (total_novel > g_max_novel) g_max_novel = total_novel;
     norm_novel = total_novel / NOVEL_MAX_ESTIMATE;
   }
 
   if (g_beta > 1e-6) {
     mmd = compute_avg_mmd_distance(indiv, seed);
     if (mmd > g_max_mmd) g_max_mmd = mmd;
     norm_mmd = mmd / MMD_MAX_ESTIMATE;
   }
 
   double fitness = g_alpha * norm_novel + g_beta * norm_mmd;
   printf("[\033[1;32mFitness\033[0m] Novel = %.4f | MMD = %.4f | Combined = %.4f\n",
         norm_novel, norm_mmd, fitness);
 ...
 void update_seed(Individual* seed, char** use_argv) {
   ...
   printf("[\033[1;36mDebug\033[0m] AFL queue contains %d paths.\n", total_paths);
 
   if (total_paths == 0) {
     fprintf(stderr, "[\033[1;31mError\033[0m] No entries in AFL queue! Cannot update seed.\n");
     exit(1);
   }
 
   int sample_count = total_paths >= 300 ? 300 : (total_paths / 2);
   if (sample_count < 1) sample_count = 1;
   printf("[\033[1;36mDebug\033[0m] Sample count set to %d\n", sample_count);
 
   destroy_individual(seed);
   // printf("[\033[1;36mDebug\033[0m] Destroyed old seed individual.\n");
 
   init_individual(seed, sample_count);
   // printf("[\033[1;36mDebug\033[0m] Initialized new seed individual with %d testcases.\n", sample_count);
   ...
   // printf("[\033[1;36mDebug\033[0m] Queue entry array built for random sampling.\n");
 
   for (int i = 0; i < sample_count; i++) {
     int chosen = rand() % total_paths;
     struct queue_entry* entry = queue_array[chosen];
 
     // printf("[\033[1;36mDebug\033[0m] Sample #%d selected: %s (%u bytes)\n", i, entry->fname, entry->len);
     ...
     // printf("[\033[1;32mSeed\033[0m] Loaded sample #%d into seed. Size: %u bytes\n", i, entry->len);
   }
 
   ck_free(queue_array);
   // printf("[\033[1;36mDebug\033[0m] Freed queue entry array.\n");
 
   // 执行所有样本，记录覆盖率
   printf("[\033[1;36mDebug\033[0m] Running seed individual to record coverage...\n");
 ...
 void check_stall_and_update(Population* pop, Individual* seed_individual, char** use_argv) {
   ...
   int novel_stalled = 0, mmd_stalled = 0;
 
   if (g_alpha > 1e-6) {
     double EPSILON_NOVEL = 0.001 * prev_max_novel;
     novel_stalled = (g_max_novel - prev_max_novel) < EPSILON_NOVEL;
     printf("[Debug] Prev Novel = %.4f, Curr = %.4f, Δ = %.6f (EPS = %.6f)\n",
           prev_max_novel, g_max_novel, g_max_novel - prev_max_novel, EPSILON_NOVEL);
   }
 
   if (g_beta > 1e-6) {
     double EPSILON_MMD = 0.001 * prev_max_mmd;
     mmd_stalled = (g_max_mmd - prev_max_mmd) < EPSILON_MMD;
     printf("[Debug] Prev MMD = %.6f, Curr = %.6f, Δ = %.6f (EPS = %.6f)\n",
           prev_max_mmd, g_max_mmd, g_max_mmd - prev_max_mmd, EPSILON_MMD);
   }
 
   // 判断是否真正停滞
   int should_stall = 0;
   if (g_alpha > 1e-6 && g_beta > 1e-6)
     should_stall = novel_stalled || mmd_stalled;  // 两个都停才算停
   else if (g_alpha > 1e-6)
     should_stall = novel_stalled;
   else if (g_beta > 1e-6)
     should_stall = mmd_stalled;
   ...
   if (stall_generations >= 5) {
     ...
     FILE* f = fopen(FITNESS_LOG_FILE, "a");
     if (f) {
         fprintf(f, "Need to update! Before update: Max Novel = %.4f | Max MMD = %.6f\n", g_max_novel, g_max_mmd);
     } else {
         perror("fopen fitness log");
     }
     ...
     // 记录更新日志
     if (f) {
         fprintf(f, "Complete update! After update: Max Novel = %.4f | Max MMD = %.6f\n", g_max_novel, g_max_mmd);
         fclose(f);
     } else {
         perror("fopen fitness log");
     }
...
size_t havoc_byte(u8* in_buf, u32 len, u8*** mutated_bufs) {
 ...
 Population* init_population(Individual* seed, int pop_size, char** use_argv) {
   ...
   printf("[\033[1;34mInfo\033[0m] Initializing population of size %d...\n", pop_size);
 
   Population* pop = ck_alloc(sizeof(Population));
   pop->count = pop_size;
 ...
 Individual* build_individual_from_queue() {
   ...
   if (count == 0) {
     fprintf(stderr, "[\033[1;31mError\033[0m] AFL queue is empty. Cannot build individual.\n");
     return NULL;
   }
 ...
 void crossover_individuals(Individual* p1, Individual* p2, Individual* child) {
   ...
   for (int i = 0; i < child->tc_count; i++) {
     ...
     //输出源数据和 child 内容
     // print_testcase_preview(i, chosen_parent, src);
   }
...
//  }
 ...
 Individual clone_individual(const Individual* src) {
   ...
   clone.coverage_r = ck_alloc(sizeof(uint8_t*) * clone.tc_count);
 
   for (int i = 0; i < clone.tc_count; i++) {
     const TestCase* s_tc = &src->testcases[i];
     TestCase* d_tc = &clone.testcases[i];
 
     d_tc->size = s_tc->size;
     d_tc->data = ck_alloc(d_tc->size);
     memcpy(d_tc->data, s_tc->data, d_tc->size);
 
     d_tc->coverage_ptr = ck_alloc(MAP_SIZE);
     memcpy(d_tc->coverage_ptr, s_tc->coverage_ptr, MAP_SIZE);
 
     d_tc->need_run = s_tc->need_run;
 
     if (s_tc->fname) {
       size_t fname_len = strlen(s_tc->fname) + 1;
       d_tc->fname = ck_alloc(fname_len);
       memcpy(d_tc->fname, s_tc->fname, fname_len);
     } else {
       d_tc->fname = NULL;
     }
 
     clone.coverage_r[i] = d_tc->coverage_ptr;
   }
  ...
  for (int i = 0; i < MAP_SIZE; i++) {
   ...
   for (int i = 0; i < offspring_count; i++) {
     ...
     do {
       p1 = rand() % SELECT_SIZE;
       p2 = rand() % SELECT_SIZE;
     } while (!allow_self_crossover && p1 == p2); // 如果不允许自交，确保 p1 ≠ p2  
   ...
   int total = pop->count + offspring_count;
   printf("[\033[1;34mMerge\033[0m] Merging %d original + %d offspring individuals...\n", pop->count, offspring_count);
 
   Individual* merged = ck_alloc(sizeof(Individual) * total);
 ...
 retry_external_pick:
   ...
   stage_max = parse_py(out_buf, len, new_buf, target->len);
 ...
 int main(int argc, char** argv) {
   ...
   u8 use_ga_mode = getenv("AFL_GA_MODE") != NULL;
   ...
   while (1) {
     ...
     if (use_ga_mode) {
       printf("[\033[1;35mGA Mode\033[0m] Environment variable AFL_GA_MODE detected. Using Genetic Algorithm fuzzing.\n");
       skipped_fuzz = fuzz_population(pop, use_argv);
     } else {
       printf("[\033[1;35mNo GA Mode\033[0m] Environment variable AFL_GA_MODE detected. Using Genetic Algorithm fuzzing.\n");
