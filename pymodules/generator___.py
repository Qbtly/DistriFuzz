...
def get_array(p, variables, param_value, param_boundary, depth):
    if p < 0.5:  # 变量
        ...
        for _ in range(array_length):
            ...
            new_value = generate_parameter({'type': value_type, 'value': param_value, 'boundary': param_boundary},
...
def get_object(p, variables, value_range, special_values, depth):
    if p < 0.5:  # 变量
        ...
        object_values = [generate_parameter(
            {'type': value_type, 'valueRange': value_range,
...
def get_number2(p):
    ...
    if p < 0.6:
        ...
        return random.randint(-100, 100)  # int
    else:
        return random.uniform(-100, 100)  # float
    

def get_array2(p, depth):
    array_length = random.randint(0, 5)  # len
    array_values = []
    for _ in range(array_length):
        value_type = random.choice(types)
...
def adjust(new_statement, intervalend_varnames, interval_end):
    ...
    for n in range(5):
        ...
        if "tmp_number" in new_statement:
            ...
            # print(new_arg)
            # print(new_statement)
        elif ", ..." in new_statement:
            new_statement = new_statement.replace(", ...", '', 1)
        # 未完待续
    # print(new_statement)
...
def get_new_statement(engine_name, new_var):  # just static
    p = random.random()
    if engine_name == 'js' and p < 0.6:  # Spidermonkey
        args = get_random_args(basic.newglobal)
        sm_func = random.choice(basic.newglobal)
