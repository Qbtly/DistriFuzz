...
def Dynamic_Reflection(rewriter, engine_path):
    ...
    with open(f"pymodules/output/output.js", "w") as js_file2:
        print(1111111111111)
        js_file2.write('hhh6hh')
        # js_file2.write(js1)
        js_file2.write("\n\n")
        js_file2.write(probed_sample)
        js_file2.close()

    avaliableVar = "\n   let variableNames = " + str(list(VariableNames)) + ";\n"

    with open(f"pymodules/output/{engine_name}/output1.js", "w") as js_file:
        js_file.write(js1)
        js_file.write("\n\n")
    ...
    for item in engine_paths:
        cmd.append(item)
    cmd.append(f"pymodules/output/{engine_name}/output1.js")
